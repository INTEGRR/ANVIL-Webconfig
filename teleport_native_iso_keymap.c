...
// VIA provides dynamic_keymap_get_keycode() and dynamic_keymap_set_keycode()
// We don't need our own storage
...
// Matrix mapping table: linear key index (0-84) -> [row, col]
// Based on the actual LAYOUT_75_iso matrix definition
const uint8_t KEY_TO_MATRIX[85][2] = {
    // Row 0 (visual): ESC, F1-F12, F13, PSCR, DEL (indices 0-15)
    {0, 0}, {1, 0}, {0, 1}, {1, 1}, {0, 2}, {1, 2}, {0, 3}, {1, 3},
    {0, 4}, {1, 4}, {0, 5}, {1, 5}, {0, 6}, {1, 6}, {0, 7}, {1, 7},

    // Row 1 (visual): GRV, 1-0, MINS, EQL, BSPC, PGUP (indices 16-30)
    {2, 0}, {3, 0}, {2, 1}, {3, 1}, {2, 2}, {3, 2}, {2, 3}, {3, 3},
    {2, 4}, {3, 4}, {2, 5}, {3, 5}, {2, 6}, {3, 6}, {3, 7},

    // Row 2 (visual): TAB, Q-P, LBRC, RBRC, PGDN (indices 31-44)
    {4, 0}, {4, 1}, {5, 1}, {4, 2}, {5, 2}, {4, 3}, {5, 3}, {4, 4},
    {5, 4}, {4, 5}, {5, 5}, {4, 6}, {5, 6}, {5, 7},

    // Row 3 (visual): CAPS, A-L, SCLN, QUOT, NUHS, ENT, HOME (indices 45-59)
    {6, 0}, {7, 0}, {6, 1}, {7, 1}, {6, 2}, {7, 2}, {6, 3}, {7, 3},
    {6, 4}, {7, 4}, {6, 5}, {7, 5}, {6, 7}, {4, 7}, {7, 7},

    // Row 4 (visual): LSFT, NUBS, Z-M, COMM, DOT, SLSH, RSFT, UP, END (indices 60-74)
    {8, 0}, {9, 0}, {8, 1}, {9, 1}, {8, 2}, {9, 2}, {8, 3}, {9, 3},
    {8, 4}, {9, 4}, {8, 5}, {9, 5}, {9, 6}, {8, 7}, {9, 7},

    // Row 5 (visual): LCTL, LGUI, LALT, SPC, RALT, FN, RCTL, LEFT, DOWN, RGHT (indices 75-84)
    {10, 0}, {11, 0}, {11, 1}, {10, 1}, {10, 5}, {11, 5}, {10, 6}, {11, 6},
    {10, 7}, {11, 7}
};

// Helper: Convert linear key index to matrix row/col
void linear_index_to_matrix(uint8_t key_index, uint8_t* row, uint8_t* col) {
    if (key_index < 85) {
        *row = KEY_TO_MATRIX[key_index][0];
        *col = KEY_TO_MATRIX[key_index][1];
    } else {
        *row = 0;
        *col = 0;
    }
