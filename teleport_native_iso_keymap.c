...
// VIA uses (layer, row, col) but we need (layer, key_index)
// We need to figure out the matrix positions from the layout
void linear_index_to_matrix(uint8_t key_index, uint8_t* row, uint8_t* col) {
    // This mapping depends on the actual keyboard matrix wiring
    // For now, we'll use a simple calculation and hope it works
    // Most 75% keyboards have ~16 columns
    *row = key_index / 16;
    *col = key_index % 16;
}
